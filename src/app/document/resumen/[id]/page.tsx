...
import { Document, Text, Page, View, Font, Svg, Line, Image, Circle } from '@react-pdf/renderer'
...
const DocumentComponent = ({consulta, tratamiento, somatometria}: DefaultComponentProps) => {
  return(
    <Document style={{minHeight:'100vh'}} title={`Resumen - ${consulta?.expand.paciente.nombre} ${consulta?.expand.paciente.apellidos} - ${consulta?.fecha.slice(0,10)}`}>
      <Page style={{padding:"50px", fontSize:12, fontFamily:'Lato', minHeight:'100vh'}} size={'LETTER'}>
        <View style={{}}>
          <View>
            <Image src={'/caduceo.png'} style={{width:75, position:'absolute'}}></Image>
            <View style={{display:'flex', alignItems:'center', gap:3, marginBottom:20, fontSize:15, fontWeight:'bold'}}>
            ...
            <View style={{display:'flex', gap:8, marginBottom:15, fontSize:12}}>
            ...
            <View style={{marginBottom:12, fontWeight:'bold'}}>
          ...
          <View style={{marginVertical:10, display:'flex', flexDirection:'column', gap:10}}>
            <Text style={{lineHeight:1.5}}>{consulta?.motivo_consulta}</Text>
            <Text style={{lineHeight:1.5}}>{consulta?.exp_fisica}</Text>
          </View>
          <Svg height="10" width="495"><Line x1="0" y1="0" x2="1200" y2="0" strokeWidth={2} stroke="rgb(150,150,150)" /></Svg>
          <View style={{textAlign:"left"}}>
            <Text style={{marginVertical:5}}>Plan: </Text>
          ...
          {/*This is my footer*/}
          <View wrap={false} style={{display:'flex', flexDirection:'row', justifyContent:"space-between", bottom:0,
          left:0, width:'100%'}}>
            ...
            <View style={{display:'flex', flexDirection:'column', gap:3, justifyContent:'center', alignItems:'center', marginTop:40}}>
              <Text style={{fontWeight:'bold'}}>MEDICO TRATANTE</Text>
              <Text >MANUEL MARIÑO SOLORZANO</Text>
            </View>
          </View>
        </View>
      </Page>
